...
MIN_YTDLP="2025.05.22"                     # required by the PO token plugin
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-4416}"
...
#   1. anonymous android_vr (4K video + range-seekable single-file audio).
#      yt-dlp skips android_vr when the session is authenticated, so the
#      user config's cookie options are stripped for this attempt.
#   2. on failure, retry with the user's cookies — android_vr auto-skips
#      and web_safari (HLS <=1080p) takes over.
...
if [ -f "\$USER_CONF" ]; then
    # Phase 1: anonymous (cookie options stripped) — android_vr.
