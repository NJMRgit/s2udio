...
pub enum StatusPropertyFile {
    ...
    /// The queue box's bottom title: `N songs / total time` in queue mode,
    /// `N Chapters / total time` in chapters mode.
    QueueBoxTitle(),
    ActiveTab,
    InputBuffer(),
    InputMode(),
...
impl TryFrom<StatusPropertyFile> for StatusProperty {
    ...
    fn try_from(value: StatusPropertyFile) -> Result<Self, Self::Error> {
        Ok(match value {
            ...
            StatusPropertyFile::QueueBoxTitle() => StatusProperty::QueueBoxTitle(),
