...
pub struct UiConfig {
    ...
    /// The text color as configured in the theme file. The blur accent
    /// derivation never touches it, so the queue and the tab lists'
    /// secondary text keep the same static look while borders / cava /
    /// controls follow the accent.
    pub list_text_color: Option<Color>,
    ...
    /// Highlight for multi-selected (marked) rows: the selection highlight
    /// effect with a lighter background (derived by `derive_theme_accents`).
    pub marked_item_style: Style,
    /// Highlight for the row under the mouse: the selection highlight effect
    /// slightly brighter (bg 0.58 vs the selection's 0.50) but dimmer than
    /// multi-selected rows (0.65). Derived by `derive_theme_accents`.
...
impl Default for UiConfigFile {
    fn default() -> Self {
        ...
        PARSING.with(|parsing| {
            ...
            parsing.set(true);
            let result = ron::from_str(include_str!("../../../assets/example_theme.ron"))
...
impl TryFrom<UiConfigFile> for UiConfig {
    ...
    fn try_from(value: UiConfigFile) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            text_color: StringColor(value.text_color.clone()).to_color()?,
            list_text_color: StringColor(value.text_color).to_color()?,
