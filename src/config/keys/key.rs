...
impl KeySequence {
    ...
    #[allow(dead_code)] // reserved: no default binding uses Ctrl+<key> today
    ...
    pub fn delete(mut self) -> Self {
        self.0.push(Key { key: KeyCode::Delete, modifiers: KeyModifiers::NONE });
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn ctrl_builder_adds_the_control_modifier_to_the_last_key() {
        let seq = KeySequence::new().tab().ctrl();
        assert_eq!(
            seq.0[0],
            Key { key: KeyCode::Tab, modifiers: KeyModifiers::CONTROL },
            "tab().ctrl() yields Ctrl+Tab"
        );
