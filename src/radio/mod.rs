...
pub fn radio_cache_path(cache_dir: Option<&Path>) -> PathBuf {
    ...
    let legacy: PathBuf = crate::config::utils::tilde_expand("~/.cache/rmpc")
        .into_owned()
        .into();
    if new.exists() {
        new
...
fn station_from_value(value: &serde_json::Value) -> Option<DirectoryStation> {
    ...
    fn f64_field(value: &serde_json::Value, key: &str) -> Option<f64> {
        match value.get(key) {
            ...
            Some(serde_json::Value::String(s)) => {
                s.trim().parse().ok().filter(|v: &f64| *v != 0.0)
            }
            _ => None,
        }
    }
...
/// simply omitted.
pub fn fetch_geo_location() -> Result<
    (crate::config::radio::GeoLocation, Option<String>),
> {
    ...
    Ok((
        crate::config::radio::GeoLocation {
            lat,
            lon,
            name,
        },
...
pub fn fetch_radio_directory(
    ...
    if let Some(code) = &country_code {
        ...
        if let Some(idx) = countries
            .iter()
            .position(|group| {
                group.top.first().is_some_and(|s| group_key(s) == user_key)
            }) && idx != 0
        {
...
fn fetch_directory() -> Result<Vec<CountryGroup>> {
    ...
    let mut groups: std::collections::BTreeMap<
        String,
        (String, Vec<DirectoryStation>),
    > = std::collections::BTreeMap::new();
    for station in stations {
        let key = group_key(&station);
        let entry = groups
            .entry(key)
            .or_insert_with(|| (station.country.clone(), Vec::new()));
...
pub fn fetch_country_states(
    country_code: &str,
    country_name: &str,
) -> Result<Vec<StateGroup>> {
    let url = format!(
...
fn ca_city_province(city: &str) -> Option<&'static str> {
    Some(
        match city {
            "toronto" | "ottawa" | "hamilton" | "london" | "mississauga" | "brampton"
            | "markham" | "vaughan" | "kitchener" | "windsor" | "kingston" | "guelph"
            | "waterloo" | "oakville" | "burlington" | "barrie" | "oshawa"
            | "st. catharines" => "Ontario",
            "montreal" | "quebec city" | "sherbrooke" | "trois-rivieres"
            | "trois rivières" | "laval" | "gatineau" => "Quebec",
            "vancouver" | "victoria" | "kelowna" | "surrey" | "burnaby" | "richmond"
            | "nanaimo" | "kamloops" | "prince george" => "British Columbia",
...
pub fn fetch_state_stations(
    country: &str,
    state: &str,
) -> Result<Vec<DirectoryStation>> {
