...
use crate::{
    ...
    shared::{
        ...
        mpd_query::{GLOBAL_STATUS_UPDATE, MpdQueryResult},
    },
};
...
fn handle_client_request(client: &mut Client<'_>, request: ClientRequest) -> Result<WorkDone> {
    match request {
        ...
        ClientRequest::Command(command) => {
            ...
            // Refresh the status right after the command completes so the
            // lyrics highlight, seekbar and state snap to the new position
            // immediately (most visibly after a seek) instead of waiting for
            // the next periodic status poll.
            match client.get_status() {
                Ok(status) => Ok(WorkDone::MpdCommandFinished {
                    id: GLOBAL_STATUS_UPDATE,
                    target: None,
                    data: MpdQueryResult::Status { data: status, source_event: None },
                }),
                Err(_) => Ok(WorkDone::None),
            }
        }
