...
static EXPECTED_MPRIS_ART: std::sync::Mutex<Option<String>> = std::sync::Mutex::new(None);
...
pub fn init(
    ...
    std::thread::Builder::new().name("work".to_owned()).spawn(move || {
        ...
        // The torrent section (engine spawn config) for torrent requests.
        let torrent_config = config.torrent.clone();
        // Jellyfin credentials (from jellytui's config file) are read per
        // request so server-side changes are picked up without a restart.
    ...
    match request {
        WorkRequest::FetchRadioDirectory { location, cache_dir } => {
            ...
            if directory.error.is_none() {
                radio::save_radio_cache(&radio::radio_cache_path(cache_dir.as_deref()), &directory);
            }
        ...
        WorkRequest::ResolveYtStreams { urls, action } => {
            ...
            Ok(WorkDone::YtStreamsResolved { info, action, failures })
        }
        WorkRequest::PlayTorrent { item, download } => {
            // M2 end-to-end: spawn the engine, add the torrent, wait
            // (round 18: open-ended — no deadline) for its metadata, pick
            // the largest playable file and hand the stream URL to the UI.
            // The wait runs on a dedicated thread so the work thread is
            // never blocked by a slow magnet; the prepared stream (or the
            // error) arrives as `WorkDone::TorrentStreamPrepared`. The
            // bandwidth gate (M3) and the picker/lifecycle (M4) refine
            // this path; `download` ("Play and Download") keeps the engine
            // downloading and the UI moves the completed file to
            // s2udio-downloads.
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    let result: Result<WorkDone> = (|| {
                        ...
                        let id = crate::core::torrent::add_torrent(&engine, item.source())
                            .map_err(|err| anyhow::anyhow!("{err}"))?;
                        // A magnet's file list only appears once peers
                ...
                .map_err(|err| anyhow::anyhow!("Failed to spawn torrent play thread: {err}"))?;
            Ok(WorkDone::None)
        }
        WorkRequest::DownloadTorrent { item, indices } => {
            // Round 21: the fresh-engine fallback of the popup's
            // download-only actions ("Download" / "Download all") — start
            // the engine, add the torrent, wait for its file list and
            // hand the running engine + the chosen files back to the UI
            // as `WorkDone::TorrentDownloadPrepared` (no playback; the
            // popup's scan is the normal path, this fires only when it is
            // genuinely gone).
            let key = item.source_key();
            let event_tx = event_tx.clone();
            let torrent_config = torrent_config.clone();
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    let result: Result<WorkDone> = (|| {
                        ...
                        if files.is_empty() {
                            return Err(anyhow::anyhow!("No playable media in this torrent"));
                        }
                ...
                .map_err(|err| anyhow::anyhow!("Failed to spawn torrent download thread: {err}"))?;
            Ok(WorkDone::None)
        }
        WorkRequest::ScanTorrent { item, cancel } => {
            // Round 17: scan the torrent up front (engine + add + metainfo
            // wait) so the popup can show the play actions and reuse the
            // engine. Round 18: the metainfo wait is open-ended and runs
            // on a dedicated thread — the work thread (album art, yt-dlp,
            // downloads, other scans) must never block on a slow magnet.
            // The popup's wait window is driven by
            // `WorkDone::TorrentScanProgress` events; Esc/close cancels
            // via `cancel` (the engine is dropped, killing rqbit).
            // Failures (dead magnet, missing binary) travel inside the
            // result — the popup shows a dim notice.
            let key = item.source_key();
            ...
            let spawn = std::thread::Builder::new()
                .name(format!(
                    "torrent-scan-{}",
                    key_in_thread.chars().take(24).collect::<String>()
                ))
                .spawn(move || {
            ...
            match spawn {
                ...
                Err(err) => Ok(WorkDone::TorrentScanned {
                    key,
        ...
        WorkRequest::FetchJellyfinArtists { view_id } => {
            let data = jellyfin_handle(jellyfin_config_file, |jf| {
                jf.artists(&view_id).map(|items| JellyfinResult::Artists { view_id, items })
            })
            .unwrap_or_else(|err| JellyfinResult::Error(err.to_string()));
            Ok(WorkDone::JellyfinFetched { id: crate::ui::panes::jellyfin::JF_ARTISTS, data })
        }
        WorkRequest::FetchJellyfinAlbums { artist_id } => {
            let data = jellyfin_handle(jellyfin_config_file, |jf| {
                jf.albums_of_artist(&artist_id)
                    .map(|items| JellyfinResult::Albums { artist_id, items })
            })
            .unwrap_or_else(|err| JellyfinResult::Error(err.to_string()));
            Ok(WorkDone::JellyfinFetched { id: crate::ui::panes::jellyfin::JF_ALBUMS, data })
        }
        ...
        WorkRequest::SaveMprisArt { url } => {
            // Download the thumbnail and write it where mpDris2 (patched)
            // looks for stream album art: `<cache_dir>/mpris-art`. Only the
            // current stream's art is written — a stale in-flight download
            // (the user switched streams) is dropped, and a failed download
            // of the current art removes the file so the media controls
            // never show an old image.
            match fetch_yt_thumbnail(&url) {
                ...
                Err(err) => {
                    ...
                    if is_expected_mpris_art(&url) {
                        let _ =
                            std::fs::remove_file(crate::ui::modals::paste::mpris_art_path(None));
                    }
                }
            }
            Ok(WorkDone::None)
        ...
        WorkRequest::SaveMpvMprisArt { url, cache_dir } => {
            // The mpv-mpris poster (shown by the MPRIS bridge while a video
            // plays): fetch the YouTube thumbnail into the poster file.
            match fetch_yt_thumbnail(&url) {
                Ok(bytes) => {
                    let path = crate::ui::modals::paste::mpv_mpris_art_path(cache_dir.as_deref());
        ...
        WorkRequest::FetchJellyfinSongs { album_id } => {
            let data = jellyfin_handle(jellyfin_config_file, |jf| {
                jf.songs_of_album(&album_id).map(|items| JellyfinResult::Songs { album_id, items })
            })
            .unwrap_or_else(|err| JellyfinResult::Error(err.to_string()));
            Ok(WorkDone::JellyfinFetched { id: crate::ui::panes::jellyfin::JF_SONGS, data })
        }
        ...
        WorkRequest::FetchJellyfinImage { item_id, fallback_item_id } => {
            let data = jellyfin_handle(jellyfin_config_file, |jf| {
                // The item's own image, falling back to e.g. the series
                // poster when the item (a season without its own art) has
                // none; the result still reports the original item id.
                match jf.fetch_image(&item_id, 600) {
                    Ok(bytes) => Ok(JellyfinResult::Image { item_id, bytes }),
                    Err(primary_err) => {
                        if let Some(fallback) = fallback_item_id {
                            jf.fetch_image(&fallback, 600)
                                .map(|bytes| JellyfinResult::Image { item_id, bytes })
        ...
        WorkRequest::YtDlpDownload { id, url, spec } => {
            let result = if let Some(spec) = spec.as_ref() {
                // A stream download (s2udio-downloads save-as): the output
                // dir comes from the spec, so no cache_dir is required.
                match ytdlp {
            ...
            Ok(WorkDone::YtDlpDownloaded { id, result, spec })
        }
...
#[cfg(test)]
mod mpris_art_guard_tests {
    use super::{is_expected_mpris_art, set_expected_mpris_art};

    #[test]
    fn only_the_expected_source_writes() {
        // No stream: nothing is current.
        set_expected_mpris_art(None);
        assert!(!is_expected_mpris_art("https://i.ytimg.com/vi/a/maxresdefault.jpg"));

        // The current stream's art is expected...
        set_expected_mpris_art(Some("https://i.ytimg.com/vi/b/maxresdefault.jpg".to_owned()));
        assert!(is_expected_mpris_art("https://i.ytimg.com/vi/b/maxresdefault.jpg"));
        // ...a previous stream's art is stale and must be skipped.
        assert!(!is_expected_mpris_art("https://i.ytimg.com/vi/a/maxresdefault.jpg"));

        // Jellyfin streams are keyed by their item id.
        set_expected_mpris_art(Some("jellyfin:abcdef0123456789abcdef0123456789".to_owned()));
        assert!(is_expected_mpris_art("jellyfin:abcdef0123456789abcdef0123456789"));
        assert!(!is_expected_mpris_art("https://i.ytimg.com/vi/b/maxresdefault.jpg"));
    }
}
