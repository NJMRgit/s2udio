...
fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::DebugInfo) => {
            handle_debug_info(args.config.as_deref(), args.address, args.password, args.theme.as_deref())?;
        }
...
fn handle_theme_current(config_arg: Option<&Path>, theme_arg: Option<&Path>) -> Result<()> {
    let config_paths = config_paths(config_arg);
    if config_paths.is_empty() {
        eprintln!("No available config path");
        std::process::exit(1);
    ...
    let theme_path = if let Some(theme_name) = config.theme {
        let theme_paths =
            theme_paths(theme_arg, &chosen_config_path, &theme_name);
...
fn handle_debug_info(
    ...
    let (config_file, config, config_path) =
        read_config_for_debuginfo(config_arg, address, password)
            .map_or_else(
                |err| {
...
fn run_tui(args: &mut Args) -> Result<()> {
    ...
    let ConfigResult { config, config_path } = read_config_and_theme(args)
        .unwrap_or_else(|err| {
            ...
            ConfigResult { config: Config::default_cli(args), config_path: None }
        });

    let mut config = config;
    // Merge key remaps and cava settings persisted by the Settings panel.
    config.apply_keybinds_override();
    config.apply_cava_override();
    // Restore the video playback preference (ask/mpv/mpd) persisted by
    // the Settings panel.
    ...
    // Restore the mpv audio/subtitle preferences persisted by the
    // Settings panel.
    ...
    // the blur watcher derives the text/border/selection colors
    // from the mode's accent on its first tick, so the persisted
    // appearance is skipped to avoid flashing stale colors at
    // launch (the UI starts on the theme defaults instead).
    ...
    if let Some(lyrics_dir) = &config.lyrics_dir {
        // Round 23: the lyrics dir is s2udio's own library — make
        // sure it exists (the fetcher creates parents on write,
        // but the index/watcher want it up front).
