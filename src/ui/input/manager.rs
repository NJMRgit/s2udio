...
impl InputManager {
    ...
    pub fn destroy_buffer(&self, id: BufferId) {
        // Only leave insert mode if the destroyed buffer was the active one.
        // A chained modal (opened from another modal's confirm callback) has
        // already become the active input, so closing the old modal must not
        // yank insert mode away from it.
