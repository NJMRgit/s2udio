...
impl<T: ScrollingState> DirState<T> {
    ...
    pub fn select(&mut self, idx: Option<usize>, scrolloff: usize) {
        ...
        // With no viewport known yet (e.g. a menu being built before its
        // first render) scrolling would push the offset past the first rows
        // and hide them (e.g. a leading group header); defer until the
        // viewport length is set.
