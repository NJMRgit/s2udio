    ...
    T: DirStackItem + std::fmt::Debug + Clone + Send + Sync + 'static,
{
    ...
    fn open(&mut self, autoplay: bool, ctx: &Ctx) -> Result<()> {
        ...
        if selected.is_file() {
            let (items, hovered_song_idx) = BrowserPane::enqueue(
                self,
                self.stack().current().items.iter().filter(|item| item.is_file()),
            );
            if !items.is_empty() {
        ...
        let (items, idx) = items
            ...
            .fold(
                ...
                |mut acc, (idx, item)| {
                    ...
                    if let Some(hovered) = hovered && hovered.is_file()
    ...
    fn handle_stack_mouse_action(&mut self, event: MouseEvent, ctx: &Ctx) -> Result<()> {
        ...
        match event.kind {
            ...
            MouseEventKind::LeftClick
            | MouseEventKind::DoubleClick if preview_area.contains(position) => {
