...
use super::Modal;
use crate::{
    config::keys::CommonAction, ctx::Ctx, mpd::commands::Song,
    shared::{
        ext::duration::DurationExt, id::{self, Id},
...
impl From<&Song> for KeyValues {
    fn from(song: &Song) -> Self {
        ...
        if let Some(title) = song.metadata.get("title") {
            result
                .extend(
                    title.iter().map(|item| vec!["Title".to_owned(), item.to_owned()]),
                );
        }
        if let Some(artist) = song.metadata.get("artist") {
            result
                .extend(
                    artist.iter().map(|item| vec!["Artist".to_owned(), item.to_owned()]),
                );
        }
        if let Some(album) = song.metadata.get("album") {
            result
                .extend(
                    album.iter().map(|item| vec!["Album".to_owned(), item.to_owned()]),
                );
        }
        let duration = song
            .duration
