...
pub fn create_save_modal<'a>(
    ...
    // as a save/add target.
    let radio_playlist = ctx.config.radio.playlist.clone();
    let playlists = ctx
        .query_sync(move |client| Ok(client.picker_playlists(&radio_playlist)?))?
        .into_iter()
        .sorted_by(|a, b| {
...
pub fn delete_from_playlist_or_show_confirmation(
    ...
    if confirmation {
        let modal = ConfirmModal::builder()
            ...
            // The narrow settings-style confirmation (52x8), not the
            // full-width default.
            .size((52, 8))
            .build();
        modal!(ctx, modal);
