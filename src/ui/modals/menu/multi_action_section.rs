...
impl Section for MultiActionSection<'_> {
    ...
    fn render(&mut self, area: Rect, buf: &mut Buffer, filter: Option<&str>, ctx: &Ctx) {
        ...
        for (idx, item) in self.items.iter_mut().enumerate() {
            ...
            let selected = self.selected_idx.is_some_and(|i| i == idx);
            ...
            // Hovering a clickable row gets the same treatment as the
            // queue-list hover (`hovered_item_style`).
            if mouse.is_some_and(|p| item_area.contains(p)) {
            ...
            item.buttons.render_with_hover(buttons_area, buf, &mut item.buttons_state, mouse);
        }
    }
