...
impl<T: MpdClient + MpdCommand + ProtoClient> MpdClientExt for T {
    ...
    fn path_to_mpd_uri(&mut self, path: &Path) -> Result<String, MpdError> {
        let Some(cfg) = &self.config() else {
            return Err(MpdError::Generic(
                "Cannot determine MPD music_directory to reference a downloaded file".to_owned(),
            ));
        };
        let music_directory = std::path::Path::new(&cfg.music_directory);
        let rel = path.strip_prefix(music_directory).map_err(|_| {
    ...
    fn replace_downloaded_stream(
        ...
        let uris: Vec<String> =
            files.iter().map(|file| self.path_to_mpd_uri(file)).collect::<Result<_, _>>()?;
        ...
        let _ = self.delete_id(song_id); // may already be gone
    ...
    fn replace_stream_in_playlist(
        ...
        let Some(idx) = entries.iter().position(|entry| entry == old_uri) else {
            return Err(MpdError::Generic(format!(
                "Stream '{old_uri}' no longer found in playlist '{name}'"
            )));
        };
...
#[cfg(test)]
mod picker_playlists_tests {
    use super::MpdClientExt;
    use crate::tests::fixtures::mpd_client::client;

    /// The radio favourites playlist is Radio-tab-owned: it never appears
    /// in the user-facing add/delete playlist pickers.
    #[test]
    fn picker_playlists_hides_the_radio_playlist() {
        let mut client = client();
        // The fixture has no radio playlist by default; simulate one.
        client.playlists.push(crate::tests::fixtures::mpd_client::TestPlaylist {
            name: "radio".to_owned(),
        ...
        // A non-matching radio name leaves everything in.
        let names = client.picker_playlists("nope").unwrap();
