...
pub trait MpdClientExt {
    fn resolve_and_enqueue(
        ...
        let (autoplay_idx, position) = match opts
            .autoplay_idx_and_queue_position(
                &ctx.queue,
                current_song_idx,
                hovered_song_idx,
            )
    ...
    /// The stored playlists a user-facing picker may offer: everything
    /// except the radio favourites playlist, which the Radio tab owns (only
    /// stations may live there — songs must never be addable to it).
    fn picker_playlists(
        &mut self,
        radio_playlist: &str,
    ...
    /// The MPD-relative URI of an absolute file path under the music
    /// directory (e.g. `music.mp3`).
    fn path_to_mpd_uri(&mut self, path: &Path) -> Result<String, MpdError>;
    /// Replace the queue entry of a downloaded stream with the downloaded
    /// file(s), keeping its queue position.
    fn replace_downloaded_stream(
        &mut self,
        files: Vec<PathBuf>,
        song_id: u32,
        position: Option<usize>,
    ) -> Result<(), MpdError>;
    /// Replace the entry `old_uri` of a stored MPD playlist with the
    /// downloaded file, keeping its position.
    fn replace_stream_in_playlist(
        &mut self,
        name: &str,
        old_uri: &str,
        file: &Path,
...
impl<T: MpdClient + MpdCommand + ProtoClient> MpdClientExt for T {
            ...
            match item {
                ...
                Enqueue::Find { filter } => {
                    self.send_find_add(
                        &filter
                            .into_iter()
                            .map(|(tag, kind, value)| Filter::new_with_kind(
                                tag,
                                value,
                                kind,
                            ))
            ...
            Ok(
                self
                    ...
                    .map(|output| PartitionedOutput {
                        id: output.id,
                        name: output.name,
                        enabled: if output.plugin == "dummy" {
                            false
                        } else {
                            output.enabled
                        },
        ...
        match result {
            ...
            Err(MpdError::Mpd(err)) if err.is_no_exist() => {
                let Some(cache_dir) = cache_dir else {
                    log::error!(
                        err:?;
                        "MPD reported error when adding files from yt-dlp cache dir, but cache_dir is not configured"
                    );
                ...
                let Some(cfg) = &self.config() else {
                    log::error!(
                        err:?;
                        "MPD reported error when adding files from yt-dlp cache dir, but cannot get music_directory from MPD"
                    );
                    return Err(MpdError::Mpd(err))?;
                };
                let music_directory = &cfg.music_directory;
                log::warn!(
                    cache_dir:?, music_directory:?;
                    "MPD reported noexist error when adding files from yt-dlp cache dir. Will try again after issuing database update."
                );
                ...
                let job_id = self
                    .update(
                        Some(
                            update_dir.to_str().context("update dir is not valid utf-8")?,
                        ),
                    )?
                    .job_id;
                loop {
    ...
    /// The MPD-relative URI of an absolute file path under the music
    /// directory (e.g. `music.mp3`), so MPD commands can
    /// reference files written there.
    fn path_to_mpd_uri(&mut self, path: &Path) -> Result<String, MpdError> {
        let Some(cfg) = &self.config() else {
            return Err(
                MpdError::Generic(
                    "Cannot determine MPD music_directory to reference a downloaded file"
                        .to_owned(),
                ),
            );
        };
        let music_directory = std::path::Path::new(&cfg.music_directory);
        let rel = path
            .strip_prefix(music_directory)
            .map_err(|_| {
                MpdError::Generic(
                    format!(
                        "Downloaded file '{}' is outside the MPD music directory '{}'",
                        path.display(), music_directory.display()
                    ),
                )
            })?;
        rel.to_str()
    ...
    /// Replace the queue entry of a downloaded stream with the downloaded
    /// file(s): drop the stream entry and insert the files at its queue
    /// position. When the entry is already gone (the queue changed while
    /// downloading), the files are appended instead. The files must be
    /// inside the MPD music directory (downloads now land in
    /// `~/Downloads/s2udio-downloads`, which is outside the library — the
    /// caller keeps the stream entry in that case).
    fn replace_downloaded_stream(
        &mut self,
        files: Vec<PathBuf>,
        song_id: u32,
        ...
        let uris: Vec<String> = files
            .iter()
            .map(|file| self.path_to_mpd_uri(file))
            .collect::<Result<_, _>>()?;
        ...
        let _ = self.delete_id(song_id);
        for (offset, uri) in uris.iter().enumerate() {
            self.add(uri, Some(QueuePosition::Absolute(position + offset)))?;
        }
        Ok(())
    }
    /// Replace the entry `old_uri` of a stored MPD playlist with the
    /// downloaded file, keeping its position in the playlist. The file
    /// must be inside the MPD music directory (see
    /// [`replace_downloaded_stream`]).
    fn replace_stream_in_playlist(
        &mut self,
        name: &str,
        old_uri: &str,
        ...
        let uri = self.path_to_mpd_uri(file)?;
        let entries = self.list_playlist(name)?.0;
        let Some(idx) = entries.iter().position(|entry| entry == old_uri) else {
            return Err(
                MpdError::Generic(
                    format!("Stream '{old_uri}' no longer found in playlist '{name}'"),
                ),
            );
        };
        self.delete_from_playlist(name, &SingleOrRange::single(idx))?;
        self.add_to_playlist(name, &uri, Some(idx))?;
        Ok(())
    }
}
