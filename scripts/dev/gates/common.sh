...
# ---------------------------------------------------------------- G7 -------
gate_g7_ytdlp_soft() {
    # Unpinned resolution probe (2026-08-22): android_vr is SABR-dead
    # (403s on consumer request shapes), so the old pinned probe was
    # retired — s2udio now relies on yt-dlp's default client order
    # (VISIONOS et al). Requires a recent yt-dlp; distro builds on
    # Debian/Ubuntu are typically too old.
    local id=""
    ...
    if [[ "$id" == "jNQXAC9IVRw" ]]; then
        write_gate G7 pass "yt-dlp $yver resolved https://www.youtube.com/watch?v=jNQXAC9IVRw (default clients) -> $id"
    else
        write_gate G7 soft "yt-dlp $yver could not resolve the test URL with default clients (distro yt-dlp too old on Debian/Ubuntu? current pipx yt-dlp resolves) — got: '${id:-<empty>}'"
    fi
}
